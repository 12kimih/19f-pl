...
module Translator = struct
  let rec trans : K.program -> Sm5.command = function
    ...
    | K.LETF (f, x, e1, e2) ->
      [Sm5.PUSH (Sm5.Fn (x, [Sm5.BIND f] @ trans_tail e1)); Sm5.BIND f] @
      trans e2 @ [Sm5.UNBIND; Sm5.POP]
  ...
  and trans_tail prog = match prog with
    | K.SEQ (e1, e2) -> trans e1 @ [Sm5.POP] @ trans_tail e2
    | K.IF (e_cond, e_true, e_false) -> trans e_cond @ [Sm5.JTR (trans_tail e_true, trans_tail e_false)]
    | K.LETV (x, e1, e2) -> trans e1 @ [Sm5.MALLOC; Sm5.BIND x; Sm5.PUSH (Sm5.Id x); Sm5.STORE] @ trans_tail e2
    | K.LETF (f, x, e1, e2) -> [Sm5.PUSH (Sm5.Fn (x, [Sm5.BIND f] @ trans e1)); Sm5.BIND f] @ trans_tail e2
    | K.CALLV (f, arg_exp) -> [Sm5.PUSH (Sm5.Id f); Sm5.PUSH (Sm5.Id f)] @ trans arg_exp @ [Sm5.MALLOC; Sm5.CALLTAIL]
    | K.CALLR (f, arg_var) -> [Sm5.PUSH (Sm5.Id f); Sm5.PUSH (Sm5.Id f); Sm5.PUSH (Sm5.Id arg_var); Sm5.LOAD; Sm5.PUSH (Sm5.Id arg_var); Sm5.CALLTAIL]
    | _ -> trans prog
end
